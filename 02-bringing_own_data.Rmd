...
::: {.notice}
This step can be skipped if gcloud storage has previously been installed in your Workspace, and you can move directly to Step Four. However, if you created your Workspace from scratch like we have in our example, you'll need to do this step.
:::

First, open a terminal window in your environment. You can access the terminal by clicking the icon below the Jupyter logo in the right-hand menu.
...
This will open a new tab in your browser window, but you are still in the same workspace! Next, run the following command in your Terminal.

`curl https://sdk.cloud.google.com | bash`

```{r, echo=FALSE, fig.alt="The terminal will open in a new window, but you are still in the same workspace."}
ottrpal::include_slide("https://docs.google.com/presentation/d/1lSUfsg_oja-Iqq5pTFD1VOuR32GLUq2l-sCBYNO3OTg/edit?slide=id.g3afa908e330_0_210#slide=id.g3afa908e330_0_210")
```

You will be asked where to create the directory. You can hit Enter to use the default.

Open a new bash shell in your terminal, or restart your shell using this command:

`exec -l $SHELL`

Finally, authenticate your gcloud by running the following:
...
You can still move data into this Workspace even if the bucket isn't listed under this project ID!
:::
...
**In order for `gcloud` to recognize the buckets, you need to add `gs://` to the front of their names.** So, for this example, the command to move the forward read file is
...
Without the `gs://`, you will get an error message that the file URL does not exist!

## Step Five: Verify that files have been transferred

But how do you know if your files were successfully transferred?
...
- Create a workspace
- Provision a Jupyter environment
- Open terminal and install `gcloud` (if not done previously)
