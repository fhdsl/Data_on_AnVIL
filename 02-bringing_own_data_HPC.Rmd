...
You will need Google Cloud SDK on your HPC to move files to AnVIL. This is software that enables file transfer. You can check to see if Google Cloud SDK tools are available by entering `gcloud` at the command line. If these tools are avaiable, you will see a list of command groups:
...
You might need to specify the path to the executable `gcloud`, e.g., `google-cloud-sdk/bin/gcloud init`. When in doubt, try testing the executable is available with `gcloud` at the command line.
...
You might have to use the command `gcloud auth login` if you have done any of these steps before. 
